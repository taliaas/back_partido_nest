...
export class BalanceService {
  ...
  //Funcion para actualizar el balance 
  async updateBalanceByActaROId(actaRoId: number): Promise<void> {
    ...
    const balance = await this.balanceRepository.findOne({ where: { minutes: actaRoId } });
    if (!balance) {
    ...
    balance.agreements_cp = actaCp ? (actaCp.agreements.match(/desc/g) || []).length : 0;
